...
def handle_user_data(user_data):
    ...
    for key, value in enumerate(values):
        if re.search(r"[0-9]{5}", value):
            p_number = value.split("\n")
            print(f"Personal number: {p_number[1]}")
            continue
        elif re.search(r"^[a-zA-Z]+$", value) and not(re.search(r"NULL", value)):
            print(f"Name: {value}")
            continue
        elif re.search(r"^[a-zA-Z ]+$", value) and not(re.search(r"NULL", value)):
            print(f"Fullname: {value}")
            continue
        elif re.search(r"[a-zA-Z0-9]+@[a-zA-Z0-9]+.[a-z]+", value):
            print(f"E-mail address: {value}")
            continue
        elif re.search(r"\d{4}-\d{2}-\d{2}", value):
            split_date = value.split("-")
            print(f"Employment start: {split_date[2]}.{split_date[1]}.{split_date[0]}")
            print("------------------------")
            continue
        elif re.search(r"[0-9]{4}", value):
        # elif re.search(r'\b\d+\b', value) and not(re.search(r'-', value)):
            weekly_fund = int(value) / 60
            print(f"Weekly fund: {weekly_fund} h.")
            continue

handle_user_data(import_text)
